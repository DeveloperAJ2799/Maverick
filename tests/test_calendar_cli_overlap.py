"""Regression: `mavrick-calendar list` must select events that OVERLAP the
...
`mavrick-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "mavrick-calendar"
    loader = importlib.machinery.SourceFileLoader("mavrick_calendar_cli", str(path))
