...
def test_vector_rag_writes_both_lanes_and_falls_back_to_fastembed(monkeypatch):
    ...
    assert "mavrick_rag_custom" not in fake.collections
...
def test_vector_rag_batch_index_continues_when_custom_lane_fails(monkeypatch, tmp_path):
    ...
    assert fake.collections["mavrick_rag_custom"].count() == 0
    assert fake.collections["mavrick_rag_fastembed"].count() == 1
...
def test_vector_rag_batch_index_reports_failure_when_all_lanes_fail(monkeypatch, tmp_path):
    ...
    assert fake.collections["mavrick_rag_custom"].count() == 0
    assert fake.collections["mavrick_rag_fastembed"].count() == 0


def test_rag_rebuild_does_not_reimport_legacy_collection(monkeypatch, tmp_path):
    fake = FakeChroma()
    legacy = fake.get_or_create_collection("mavrick_rag", metadata={"hnsw:space": "cosine"})
    legacy.add(
        ids=["stale-doc"],
        embeddings=[[0.0] * 384],
    ...
    inactive_custom = fake.get_or_create_collection("mavrick_rag_custom", metadata={"embedding_lane": "custom"})
    inactive_custom.add(
    ...
    assert "mavrick_rag" not in fake.collections
    assert "mavrick_rag_custom" not in fake.collections
    assert fake.collections["mavrick_rag_fastembed"].count() == 0
...
def test_rag_remove_directory_deletes_inactive_lane_collection(monkeypatch, tmp_path):
    fake = FakeChroma()
    legacy_collection = fake.get_or_create_collection("mavrick_rag", metadata={"hnsw:space": "cosine"})
    custom_collection = fake.get_or_create_collection("mavrick_rag_custom", metadata={"embedding_lane": "custom"})
    fast_collection = fake.get_or_create_collection("mavrick_rag_fastembed", metadata={"embedding_lane": "fastembed"})
    ...
    fast_lane = EmbeddingLane(
        ...
        collection_name="mavrick_rag_fastembed",
        model="mini",
        url="local://fastembed",
        dimension=384,
        fingerprint="fast",
...
def test_vector_rag_uses_keyword_fallback_when_all_lanes_query_fail():
    collection = FakeCollection("mavrick_rag_fastembed", metadata={"embedding_lane": "fastembed"})
    collection.add(
        ids=["doc-1"],
        embeddings=[[0.0] * 384],
