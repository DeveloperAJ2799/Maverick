...
base compose with only the matching overlay merged into the ``mavrick``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on mavrick only."""
...
# --- Non-mavrick services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_mavrick_services_match_base(base, standalone_path):
...
# --- mavrick = base service + only the overlay additions ------------------


def test_nvidia_mavrick_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_mavrick_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
...
def test_base_has_no_host_docker_access(base):
    ...
    assert "MAVRICK_ENABLE_HOST_DOCKER=true" not in service["environment"]
...
def test_base_plus_host_docker_overlay_has_explicit_access(base):
    ...
    assert "MAVRICK_ENABLE_HOST_DOCKER=true" in service["environment"]
