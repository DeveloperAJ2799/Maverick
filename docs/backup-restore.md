...
Mavrick keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/mavrick-backup` tool
...
```bash
# Create a snapshot → backups/mavrick-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/mavrick-backup snapshot

# List existing snapshots (most recent first)
./scripts/mavrick-backup list

# Check a tarball's integrity without extracting it
./scripts/mavrick-backup verify backups/mavrick-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/mavrick-backup restore backups/mavrick-backup-20260101-120000.tar.gz --yes
```
...
./scripts/mavrick-backup snapshot --out /mnt/nas/mavrick-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/mavrick-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/mavrick && ./scripts/mavrick-backup snapshot --out "/mnt/nas/mavrick-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `mavrick-backup` therefore does not capture the Docker
