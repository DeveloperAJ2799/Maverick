...
<#
  Build a portable Windows distribution for Mavrick.

  Output layout:
    dist\Mavrick\Mavrick.exe
    dist\Mavrick\static\...
    dist\Mavrick\scripts\...
    dist\Mavrick\mcp_servers\...
    dist\Mavrick\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Mavrick @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Mavrick" -ForegroundColor Green
