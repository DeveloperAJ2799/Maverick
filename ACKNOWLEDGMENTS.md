...
Mavrick stands on the shoulders of a lot of open-source work. This file
...
- **[Tongyi DeepResearch](https://github.com/Alibaba-NLP/DeepResearch)** by
  **Alibaba-NLP / Tongyi Lab** — the multi-step deep-research agent pipeline.
  Copyright © Alibaba-NLP / Tongyi Lab. **Apache-2.0.** Adapted for Mavrick's
...
and run alongside Mavrick on `docker compose up`. They are not modified —
just composed.
...
Mavrick talks to these over the network/API. They are **not** distributed
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  Mavrick uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of Mavrick's code was written *with* AI models, not just by a human.
