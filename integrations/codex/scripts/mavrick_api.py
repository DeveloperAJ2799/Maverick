#!/usr/bin/env python3
"""Small Mavrick scoped API helper for Codex terminal sessions."""
...
def _usage() -> int:
    ...
    print("  mavrick_api.py capabilities", file=sys.stderr)
    print("  mavrick_api.py todos list", file=sys.stderr)
    print("  mavrick_api.py todos add TITLE", file=sys.stderr)
    print("  mavrick_api.py emails list [limit]", file=sys.stderr)
    print("  mavrick_api.py emails read UID", file=sys.stderr)
    print("  mavrick_api.py emails draft-doc JSON_PAYLOAD", file=sys.stderr)
    print("  mavrick_api.py documents list [limit]", file=sys.stderr)
    print("  mavrick_api.py documents read DOC_ID", file=sys.stderr)
    print("  mavrick_api.py documents create JSON_PAYLOAD", file=sys.stderr)
    print("  mavrick_api.py documents delete DOC_ID", file=sys.stderr)
    print("  mavrick_api.py cookbook tasks", file=sys.stderr)
    print("  mavrick_api.py cookbook servers", file=sys.stderr)
    print("  mavrick_api.py cookbook cached [HOST]", file=sys.stderr)
    print("  mavrick_api.py cookbook presets", file=sys.stderr)
    print("  mavrick_api.py cookbook output SESSION_ID [tail]", file=sys.stderr)
    print("  mavrick_api.py cookbook serve REPO_ID 'CMD' [REMOTE_HOST]", file=sys.stderr)
    print("  mavrick_api.py cookbook preset NAME", file=sys.stderr)
    print("  mavrick_api.py cookbook adopt SESSION_ID MODEL [HOST] [PORT]", file=sys.stderr)
    print("  mavrick_api.py cookbook stop SESSION_ID", file=sys.stderr)
    print("  mavrick_api.py METHOD /api/codex/path [json-body]", file=sys.stderr)
    return 2


def _config() -> tuple[str, str] | None:
    base_url = os.environ.get("MAVRICK_URL", "").strip().rstrip("/")
    token = os.environ.get("MAVRICK_API_TOKEN", "").strip()
    missing = []
    if not base_url:
        missing.append("MAVRICK_URL")
    if not token:
        missing.append("MAVRICK_API_TOKEN")
    if missing:
        print(f"missing {', '.join(missing)}; create a Codex Agent token in Mavrick Settings", file=sys.stderr)
        return None
...
def main() -> int:
    ...
    if not path.startswith("/api/codex/"):
        print("refusing non-/api/codex path; use scoped Mavrick integration endpoints only", file=sys.stderr)
        return 2
