                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Mavrick (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Mavrick" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "mavrick_kind" if "mavrick_kind" in cols else "'scheduled' AS mavrick_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                mavrick_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Mavrick-Origin"] = "mavrick-ui"
                outer["X-Mavrick-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", mavrick_kind or "scheduled")[:64]
                outer["X-Mavrick-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `mavrick-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `MAVRICK_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `mavrick-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("MAVRICK_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `MAVRICK_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (MAVRICK_INPROCESS_POLLERS=0); "
            "drive `mavrick-mail poll-scheduled` externally."
        )
        return
    import asyncio
