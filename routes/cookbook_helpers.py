...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Mavrick runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Mavrick' own interpreter directory to PATH. If Mavrick itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[mavrick] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'MAVRICK_USER_SHELL="${SHELL:-}"',
        'if [ -n "$MAVRICK_USER_SHELL" ] && [ -x "$MAVRICK_USER_SHELL" ]; then',
        '  MAVRICK_USER_PATH="$("$MAVRICK_USER_SHELL" -ic \'printf "__MAVRICK_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__MAVRICK_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$MAVRICK_USER_PATH" ]; then export PATH="$MAVRICK_USER_PATH:$PATH"; fi',
        'fi',
        ...
        '_mavr_py3="$(command -v python3 2>/dev/null || true)"',
...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if os.name == 'nt' and not os.environ.get('MAVRICK_ALLOW_OLLAMA_CLI_SCAN'): return",
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Mavrick.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$MAVRICK_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $MAVRICK_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$MAVRICK_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Mavrick-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('MAVRICK_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$MAVRICK_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  MAVRICK_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[mavrick] vLLM CLI: $MAVRICK_VLLM_BIN"')
    runner_lines.append('  MAVRICK_VLLM_VERSION="$("$MAVRICK_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$MAVRICK_VLLM_VERSION" ]; then echo "[mavrick] vLLM version: $MAVRICK_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('MAVRICK_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $MAVRICK_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $MAVRICK_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $MAVRICK_CMD_EXIT ==="')
        runner_lines.append('exit "$MAVRICK_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _mavrick_have_prebuilt=1
    ...
    runner_lines.append('    _mavrick_have_prebuilt=""')
    runner_lines.append('    _mavrick_arch="$(uname -m)"')
    runner_lines.append('    _mavrick_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_mavrick_arch" = "x86_64" ]; then')
    runner_lines.append('      _mavrick_pat=""')
    runner_lines.append('      _mavrick_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _mavrick_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _mavrick_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _mavrick_has_nv_inline; then')
    runner_lines.append('        _mavrick_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _mavrick_has_vkdev_inline && _mavrick_has_vk_inline; then')
    runner_lines.append('        _mavrick_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _mavrick_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _mavrick_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_mavrick_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_mavrick_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[mavrick] Found prebuilt llama-server: $_mavrick_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/mavrick/llama-cpp-prebuilt" && cd "$HOME/.cache/mavrick/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_mavrick_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _mavrick_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_mavrick_extracted" ]; then')
    runner_lines.append('          chmod +x "$_mavrick_extracted"')
    runner_lines.append('          ln -sf "$_mavrick_extracted" ~/bin/llama-server')
    runner_lines.append('          _mavrick_libdir="$(dirname "$_mavrick_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_mavrick_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/mavrick-llama-cpp-env')
    runner_lines.append('          _mavrick_have_prebuilt=1')
    runner_lines.append('          echo "[mavrick] Prebuilt llama-server installed at $_mavrick_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_mavrick_have_prebuilt" ] && echo "[mavrick] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_mavrick_prebuilt_url" ]; then')
    runner_lines.append('      echo "[mavrick] No matching prebuilt llama-server for this host (arch=$_mavrick_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_mavrick_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _mavrick_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _mavrick_has_nvidia_hw; then')
    ...
    runner_lines.append('    _mavrick_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[mavrick] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[mavrick] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[mavrick] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[mavrick] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _mavrick_apt_bootstrap')
    runner_lines.append('    _mavrick_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _mavrick_missing_build_deps="$_mavrick_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _mavrick_missing_build_deps="$_mavrick_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _mavrick_missing_build_deps="$_mavrick_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_mavrick_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_mavrick_missing_build_deps"')
    ...
    runner_lines.append('    _mavrick_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _mavrick_has_vulkan_device() {')
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[mavrick] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _mavrick_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      _mavrick_has_cudart() {')
    ...
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    runner_lines.append('      if _mavrick_has_cudart; then')
    runner_lines.append('        echo "[mavrick] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[mavrick] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[mavrick]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[mavrick]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _mavrick_has_vulkan_device && _mavrick_has_vulkan; then')
    runner_lines.append('      echo "[mavrick] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[mavrick] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[mavrick]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[mavrick]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _mavrick_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[mavrick] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[mavrick] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[mavrick] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\mavrick-sessions"
