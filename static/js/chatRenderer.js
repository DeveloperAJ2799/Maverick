...
const _ACTION_RECENTS_KEY = 'mavrick-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'mavrick-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Mavrick';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Mavrick' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
