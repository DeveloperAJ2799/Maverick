...
let _sortMode = Storage.get('mavrick-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Mavrick Chat';
...
const FOLDER_STATE_KEY = 'mavrick-folder-state';
const FOLDER_ORDER_KEY = 'mavrick-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Mavrick kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Mavrick Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('mavrick-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('mavrick-session-sort', mode);
  else Storage.remove('mavrick-session-sort');
  renderSessionList();
}
