...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      'Mavrick Cookbook hardware diagnostics',
      ...
      '',
      'Useful checks:',
      'docker compose exec mavrick nvidia-smi -L',
      'docker compose exec mavrick cat /proc/meminfo | head',
      'docker compose exec mavrick python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    // dropdown still showed mavrick. The user's selection must only change via
