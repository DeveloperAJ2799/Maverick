// ============================================
// Mavrick UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__mavrickDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Mavrick Chat';
  ...
  if (_opener) window._mavrickRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('mavrick-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__mavrickSetChatMode = setMode;
  ...
  const SPLASH_COUNT_KEY = 'mavrick-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message Mavrick...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__mavrickSetChatMode === 'function') {
          window.__mavrickSetChatMode('chat');
        } else {
        ...
        if (typeof window.__mavrickSetChatMode === 'function') {
          window.__mavrickSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'mavrick-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'mavrick-toolbar-visibility';
    try {
...
// ============================================
function startMavrickApp() {
  if (window.__mavrickAppStarted) return;
  window.__mavrickAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__mavrickChatBusyUntil = Math.max(window.__mavrickChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('mavrick-doc-open-' + _curSession) === '1') {
  ...
  window._mavrickBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._mavrickRouteOpener) {
          try { window._mavrickRouteOpener(); } catch (_) {}
          window._mavrickRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startMavrickApp, { once: true });
} else {
  startMavrickApp();
}
